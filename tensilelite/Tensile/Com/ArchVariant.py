...
from typing import NamedTuple, Optional, List, Union, Tuple, Set
...
class ArchVariant(NamedTuple):
    Name: str
    Gfx: str
    DeviceIds: Optional[Set[str]] = None
...
def extractArchVariant(file: Union[str, Path]) -> ArchVariant:
    ...
    def l2(line: str):
        if match := re.match(r'- \{Architecture: (\w+), CUCount: (\d+)\}', line):
            ...
            raise LogicFileError(f"Expected architecture and CU count, or only an archiecture: line: {line}")
...
def parseArchVariantString(spec: str) -> Tuple[Set[str], Set[int]]:
    ...
    Raises:
        ValueError: If the specification string is invalid for the following reasons:
            - The device ID is not a 4-digit number.
    ...
    if spec:
        for s in spec.strip().split(";"):
            ...
            value = value.strip()
            if key == "id" and value.isdigit() and len(value) == deviceIdLength:
                deviceIds.add(value)
...
def matchArchVariant(gfxNames: Set[str], deviceIds: Set[str], cuCounts: Set[int], targetLogicFile: Union[str, Path]):
    ...
    conditions = [
        ...
        any(id in deviceIds for id in variant.DeviceIds) or variant.DeviceIds == None,
