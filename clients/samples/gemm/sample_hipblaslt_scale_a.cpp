...
public:
          ...
          max_workspace_size(max_workspace_size)
    {
        ...
        h_a_scale = static_cast<hipblasLtHalf>(2.0f);
    ...
    void cpu_gemm()
    {
        for (int64_t i = 0; i < m; ++i)
        {
            for (int64_t j = 0; j < n; ++j)
            {
                ...
                temp *= static_cast<double>(h_a_scale);
...
void simpleGemmWithAScale(hipblasLtHandle_t  handle,
                          ...
                          hipblasLtHalf*     d_a_scale)
{
    hipblasLtHalf h_a_scale_verify;
    CHECK_HIP_ERROR(hipMemcpy(&h_a_scale_verify, d_a_scale, sizeof(hipblasLtHalf), hipMemcpyDeviceToHost));
    std::cout << "A Scale factor on device: " << static_cast<float>(h_a_scale_verify) << std::endl;
    std::cout << "----------------------------" << std::endl;
    ...
    CHECK_HIPBLASLT_ERROR(hipblasLtMatmulDescSetAttribute(
        matmul, HIPBLASLT_MATMUL_DESC_A_SCALE_POINTER, &d_a_scale, sizeof(hipblasLtHalf*)));

    hipblasLtHalf* scale_ptr_a = nullptr;
    ...
    CHECK_HIPBLASLT_ERROR(hipblasLtMatmulDescGetAttribute(
        ...
        &scale_ptr_a, scale_ptr_size, &size_written));
    std::cout << "A Scale pointer set: " << (scale_ptr_a == d_a_scale ? "Yes" : "No") << std::endl;
    std::cout << "----------------------------" << std::endl;
...
int main()
{
    ...
    runner.run([&runner] {
        simpleGemmWithAScale(runner.handle, HIPBLAS_OP_N, HIPBLAS_OP_N,
                             ...
                             runner.stream, runner.d_a_scale);
    });
